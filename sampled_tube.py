"""Helpers for constructing empirical reachable tubes from sampled rollouts."""
...
def _decoder_weights(config: dict[str, Any]) -> str:
    ...
    if isinstance(weights, dict):
        weights = weights[_decoder_variant(config)]
...
def validate_cellwise_setup(
    ...
    """Validate the configs that jointly define one cellwise rollout."""
    if wrapper.get("sampling_mode") != "cellwise_tube":
        raise ValueError("sampling_mode must be 'cellwise_tube'")
    ...
    model_id = _decoder_variant(sampling_config)
    if str(wrapper["model_id"]) != model_id:
        raise ValueError(
            f"model_id mismatch: wrapper={wrapper['model_id']!r}, "
