...
def descending_p95(scores: Sequence[float]) -> float:
    """Legacy Table-III summary statistic for exactly 400 test scores."""
...
def finite_sample_signed_margin(
    scores: Sequence[float],
    alpha: float,
) -> tuple[float, int]:
    """Return the conformal signed-margin order statistic for any valid n."""

    values = np.asarray(scores, dtype=float)
    if values.size == 0:
        raise ValueError("calibration requires at least one valid score")
    if not np.all(np.isfinite(values)):
        raise ValueError("trajectory scores must be finite")
    ...
    if rank > values.size:
        raise ValueError(
            f"{values.size} calibration trajectories are insufficient "
            f"for finite alpha={alpha}; need at least "
            f"{math.ceil(1.0 / alpha) - 1}"
        )
    return float(np.sort(values)[::-1][rank - 1]), rank
...
def calibration_gamma(
    ...
    """Calibrate one scalar inflation from a held-out split of any size."""
    ...
    if len(scores) != len(rows):
        raise ValueError(
            "all calibration trajectories must match a valid tube cell, "
            f"got valid={len(scores)}, total={len(rows)}"
        )
    rank_margin, rank = finite_sample_signed_margin(scores, alpha)
    return max(0.0, -rank_margin), rank_margin, rank
...
def build_tube_variants(
    *,
    ...
    cp_r_gamma, cp_r_rank_margin, cp_r_rank = calibration_gamma(
        calibration_trajectories, grid, cells, dims, alpha
    )
    variants = {
        "Raw tube": cells,
    ...
    cp_r = {
        "method": "finite-sample conformal signed trajectory margin",
        "n": int(len(calibration_trajectories)),
        "alpha": float(alpha),
        "sort": "descending",
        "rank": int(cp_r_rank),
